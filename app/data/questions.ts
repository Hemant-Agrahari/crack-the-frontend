...
export const questions: Question[] = [
  ...
  },
  {
    ...
    answer:
      "Object.freeze and Object.seal are both used to restrict changes to JavaScript objects, but freeze is more strict. Object.freeze makes an object completely immutable, meaning you cannot add new properties, delete existing ones, or modify any values. Object.seal, on the other hand, locks only the structure of the object. You cannot add or remove properties, but you can still change the values of existing properties. In simple terms, freeze locks both structure and values, while seal locks only the structure.",
    category: "javascript",
  },
  {
    id: 76.2,
  ...
  // ========================================
  {
    ...
    answer:
      "Both map and forEach are used to iterate over arrays in JavaScript, but they serve different purposes. forEach is mainly used when you just want to loop through an array and perform some action like logging values, updating variables, or making API calls. It does not return anything and always returns undefined. On the other hand, map is used when you want to transform each element of an array and get a new array as a result. It returns a new array with the same length, where each element is the transformed version of the original. In short, forEach is for side effects, while map is for creating a new array.",
    category: "javascript",
  },
  {
    id: 248510,
  ...
  },
  {
    ...
    answer:
      "Both for...in and for...of are looping statements in JavaScript, but they are used for different purposes. for...in is mainly used to iterate over the keys or property names of an object. It gives you the index or key as a string and is generally not recommended for arrays because it can include inherited properties. On the other hand, for...of is used to iterate over the actual values of iterable data structures like arrays, strings, maps, sets, and NodeLists. It directly returns each value, making it the preferred choice for looping over arrays. In simple terms, for...in is used for object keys, while for...of is used for values of arrays and other iterables.",
    category: "javascript",
  },
  // ========================================
  ...
  },
  {
    ...
    answer:
      "These Promise methods are used to handle multiple asynchronous operations, but they differ in behavior. Promise.all executes all promises in parallel and resolves only when every promise succeeds; if any promise rejects, it immediately fails. Promise.race returns the result of the first promise that settles, whether it resolves or rejects, so the fastest one decides the outcome. Promise.allSettled waits for all promises to finish and returns the status and value of each promise, regardless of success or failure, and it never rejects. In short, Promise.all is best when all tasks must succeed, Promise.race is useful when only the first result matters, and Promise.allSettled is ideal when you need the result of every promise.",
    category: "javascript",
  },
  {
    id: 67,
  ...
  },
  {
    ...
    answer:
      "Rest and Spread operators use the same three-dot syntax (...) but serve opposite purposes. The Spread operator is used to expand elements of an array or object, such as copying data, merging arrays or objects, or passing multiple values into a function. On the other hand, the Rest operator is used to collect multiple values into a single array, commonly in function parameters or destructuring. In simple terms, spread breaks data apart, while rest gathers data together, making code cleaner and more flexible.",
    category: "javascript",
  },
  {
    id: 109.1,
  ...
  },
  {
    ...
    answer:
      "Debounce and throttling are performance optimization techniques used to control how frequently a function executes. Throttling ensures that a function runs at regular intervals, even if an event like scrolling or resizing keeps firing continuously. This is useful when you want consistent updates without overloading the system. Debouncing, on the other hand, delays the function execution until a certain amount of time has passed since the last event. If the event keeps occurring, the function never runs until the activity stops. In simple terms, throttling runs a function at fixed intervals, while debouncing runs it only after the user stops triggering the event.",
    category: "javascript",
  },
  {
    id: 9876587874,
    question:
      "How does the Event Loop work? Which has more priority: the macro task queue or the micro task queue? If there are 100 macro tasks and 1 micro task, which will execute first?",
    answer:
      "The Event Loop allows JavaScript to handle asynchronous operations while running on a single thread. When the call stack becomes empty, the Event Loop checks the task queues and decides what to execute next. Micro tasks, such as Promise callbacks, have higher priority than macro tasks like setTimeout or DOM events. After every execution cycle, the Event Loop always clears the entire micro task queue before picking the next macro task. So even if there are 100 macro tasks and only 1 micro task, the micro task will execute first.",
    category: "javascript",
  },
  {
    id: 77465656421,
    question: "How would you debug a production issue without writing logs?",
    answer:
      "When debugging a production issue without writing logs, I would first try to reproduce the issue by understanding the exact user flow, browser, device, and conditions under which the problem occurs. I would use browser developer tools to inspect network requests, API responses, headers, and payloads to check for failures or unexpected data. Tools like error monitoring services, stack traces, and performance metrics help identify where the issue is happening. I would also use feature flags or environment-based configurations to isolate problematic code paths and compare behavior between working and failing scenarios. In short, by reproducing the issue, analyzing network and runtime behavior, and using monitoring and debugging tools, we can identify and fix production issues even without adding logs.",
    category: "react",
  },
  {
    id: 884517672,
    question:
      "If the browser cache becomes full due to your application storing too much data, how would you prevent the application from crashing or breaking? How can you debug and stop a React component from re-rendering multiple times due to changes in another component?",
    answer:
      "If the browser cache or storage becomes full, I would first limit how much data the application stores by avoiding unnecessary or large payloads and clearing outdated or unused data regularly. I would use appropriate storage options like sessionStorage instead of localStorage when persistence is not required, implement size checks before saving data, and add error handling around storage operations to gracefully handle quota-exceeded errors. For caching strategies, I would also rely on HTTP caching and service workers instead of storing everything in browser storage. \n\nTo debug and stop unnecessary re-renders in React, I would use React DevTools to identify which components are re-rendering and why. I would check for unnecessary state updates, prop changes, or new object and function references being passed on each render. To fix this, I would memoize components using React.memo, stabilize functions with useCallback, memoize derived values with useMemo, and avoid lifting state higher than necessary. In short, controlling storage usage prevents crashes, and controlling state, props, and references helps stop unwanted React re-renders.",
    category: "react",
  },
  ...
  // ========================================
  {
    ...
    answer:
      "Layout thrashing occurs when JavaScript repeatedly forces the browser to recalculate layout by mixing DOM reads and writes in an inefficient way. For example, when code changes the DOM and immediately reads layout-related properties like offsetHeight or scrollWidth, the browser is forced to perform multiple reflows and repaints, which significantly hurts performance. This usually happens due to frequent DOM manipulations, measuring layout values too often, or updating styles one by one. To prevent forced reflows, you should batch DOM reads and writes separately, use CSS classes instead of multiple inline style changes, rely on requestAnimationFrame for visual updates, cache layout values when possible, and prefer CSS transitions or animations over JavaScript-based animations. In short, avoiding unnecessary layout calculations and letting the browser optimize rendering helps prevent layout thrashing.",
    category: "javascript",
  },
  {
    id: 1256104,
  ...
  },
  {
    ...
    answer:
      "Modern JavaScript engines automatically manage memory using garbage collection, so developers don’t need to free memory manually. Most engines, like V8, use a technique called mark-and-sweep. The garbage collector starts from root objects such as global variables and the current call stack, then marks all objects that are still reachable through references. Any object that cannot be reached is considered unused and is removed from memory. An object becomes unreachable when no variable or reference points to it, for example when it is set to null. To keep performance smooth, modern engines also use optimizations like generational garbage collection, incremental and concurrent cleanup, and running garbage collection during idle time.",
    category: "javascript",
  },
  ...
  // ========================================
  {
    ...
    answer:
      "Webpack and Turbopack are both bundlers used in modern JavaScript applications, but they differ mainly in performance and architecture. Webpack is a widely used and mature bundler written in JavaScript that supports features like loaders, plugins, code splitting, and tree shaking, but it can become slow as projects grow larger. Turbopack, on the other hand, is a next-generation bundler developed by Vercel and written in Rust. It is designed for speed and provides extremely fast builds and instant updates, especially in development mode. Turbopack is optimized for modern frameworks like Next.js and focuses on incremental builds for a better developer experience. In short, Webpack is powerful and battle-tested, while Turbopack is a modern, high-performance bundler built for speed and scalability.",
    category: "javascript",
  },
  {
    id: 70.98754333333,
...
So useEffect gives us mount, update, and unmount behavior in a single hook`,
  ...
  },
  {
    ...
    answer:
      "map and filter are both array methods used to process arrays, but they serve different purposes. The map method transforms each element in an array and returns a new array of the same length. It is used when you want to modify or transform elements. For example, if you have an array of numbers [1, 2, 3] and you want to double each number, map would return [2, 4, 6]. Every element is processed and the resulting array has the same number of elements as the original.\n\nOn the other hand, the filter method filters elements based on a condition and returns a new array containing only the elements that pass the test. The resulting array length can be less than the original array because it only includes elements that meet the specified condition. For instance, if you have an array [1, 2, 3, 4] and you want to get only even numbers, filter would return [2, 4]. The key difference is that map transforms every element and maintains the array length, while filter selects specific elements based on a condition and can return a shorter array. In short, use map when you need to transform data, and use filter when you need to select specific items from an array.",
    category: "javascript",
  },
  {
    id: 2003,
    question: "What is the difference between map() and forEach()?",
    answer:
      "map() and forEach() are both used to iterate over arrays, but they have different purposes and return values. The map method transforms each element and returns a new array without modifying the original array. It returns an array of transformed values and can be chained with other array methods. For example, if you have an array [1, 2, 3] and use map to double each number, it will return a new array [2, 4, 6] that you can store in a variable or chain with other methods.\n\nIn contrast, forEach executes a function for each element but returns undefined, which means it doesn't give you anything back. It cannot be chained with other array methods and is primarily used for side effects like logging to the console, updating the DOM, or performing operations where you don't need a return value. For instance, if you use forEach to log doubled values of [1, 2, 3], it will print 2, 4, 6 to the console but the forEach itself returns undefined. The key difference is that map returns a new array with transformed values, while forEach returns nothing and is used when you want to perform an action on each element without creating a new array. In short, use map when you need a new transformed array, and use forEach when you just need to execute code for each element without caring about the return value.",
    category: "javascript",
  },
  {
    id: 2004,
    ...
    answer:
      "Pure and Impure functions differ in their behavior and side effects. Pure functions always return the same output for the same input, have no side effects, and don't depend on external variables. They are predictable and easy to test. For example, a simple add function that takes two parameters and returns their sum is a pure function because calling it with the same arguments will always produce the same result, and it doesn't modify anything outside its scope.\n\nImpure functions, on the other hand, may return different outputs for the same input and have side effects such as modifying external state, variables, the DOM, or making API calls. They depend on external factors, which makes them harder to test and predict. For instance, an increment function that modifies a global count variable is impure because it changes external state and calling it multiple times with no arguments will return different results each time. The key difference is that pure functions are deterministic and safe with no side effects, while impure functions modify state or depend on external factors, making their behavior less predictable. Pure functions are preferred in functional programming because they are easier to understand, test, and debug.",
    category: "javascript",
  },
  {
    id: 2005,
    ...
    answer:
      "for-in and for-of are both looping constructs in JavaScript, but they iterate over different things. The for-in loop iterates over enumerable property keys or indexes and is used primarily for objects. It returns property names or keys as strings and also iterates over inherited properties. For example, when you use for-in on an object with properties a and b, it will give you the keys 'a' and 'b'. When used on an array, it returns the indexes as strings like '0', '1', etc.\n\nThe for-of loop, on the other hand, iterates over iterable values rather than keys. It is used for arrays, strings, Maps, Sets, and NodeLists, and it returns the actual values rather than keys or indexes. However, it cannot be used directly on plain objects because objects are not iterable by default. For instance, when you use for-of on an array containing 'x' and 'y', it will give you the values 'x' and 'y' directly. Similarly, when used on a string like 'hi', it will give you each character 'h' and 'i'. The key difference is that for-in loops over keys or properties, while for-of loops over values. In short, use for-in when you need to iterate over object keys, and use for-of when you need to iterate over values in iterable collections like arrays and strings.",
    category: "javascript",
  },
  {
    id: 2006,
    question: "What are DRY, KISS, YAGNI, SOLID Principles?",
    answer:
      "These are software development principles that guide clean, maintainable code. DRY stands for Don't Repeat Yourself, which means you should avoid code duplication by extracting repeated logic into reusable functions or components. Instead of copying the same code multiple times, create a utility function that can be called whenever needed. KISS stands for Keep It Simple, Stupid, which emphasizes keeping code simple and straightforward while avoiding unnecessary complexity. Write code that others can easily understand by using clear variable names and avoiding over-engineering solutions.\n\nYAGNI stands for You Aren't Gonna Need It, which advises against adding functionality until it's actually needed. This principle helps avoid premature optimization and encourages focusing on current requirements rather than hypothetical future needs. SOLID is actually an acronym for five principles primarily used in object-oriented programming. The S stands for Single Responsibility Principle, meaning one class or function should have one purpose. The O stands for Open/Closed Principle, which states that code should be open for extension but closed for modification. The L represents Liskov Substitution Principle, which means subtypes must be substitutable for their base types. The I stands for Interface Segregation Principle, suggesting many small interfaces are better than one large interface. Finally, the D represents Dependency Inversion Principle, which states you should depend on abstractions rather than concrete implementations. Together, these principles promote clean, maintainable, scalable, and testable code.",
    category: "javascript",
  },
  {
    id: 2007,
    ...
    answer:
      "JavaScript provides multiple ways to create objects. The most common way is using object literal syntax, where you simply define an object with curly braces and key-value pairs, like const obj = { name: 'John', age: 25 }. Another way is using the new Object() constructor, where you first create an empty object and then add properties to it one by one. You can also use constructor functions, which are regular functions that you call with the new keyword to create multiple objects with the same structure. For example, you can define a Person function that takes name and age parameters and assigns them using the this keyword.\n\nObject.create() is another method that allows you to create a new object with a specified prototype object, which is useful for inheritance patterns. ES6 introduced the class syntax, which provides a cleaner way to create objects using the class keyword and constructor method, though it's essentially syntactic sugar over constructor functions. Factory functions are regular functions that return new objects without using the new keyword, making them simpler and more flexible. Finally, you can use Object.assign() to create objects by merging multiple source objects into a target object. Each method has its use case, with object literals being the most common for simple objects, while constructor functions and classes are better for creating multiple similar objects with shared behavior.",
    category: "javascript",
  },
  {
    id: 2008,
    ...
    answer:
      "A polyfill is a piece of code, usually JavaScript, that provides modern functionality to older browsers that don't natively support it. Polyfills are used to make your code work across different browsers, allowing you to use modern JavaScript features in older environments while ensuring backward compatibility. They work by first checking if a particular feature exists in the browser, and if it doesn't, they implement that feature manually using older JavaScript code that the browser does understand.\n\nFor example, older browsers might not support the Array.prototype.includes() method. A polyfill for this would check if the includes method exists, and if not, it would create that method by using the indexOf() method which is supported in older browsers. Common polyfills include those for Promises in older browsers, the fetch() API, array methods like find, findIndex, and includes, and Object.assign(). Popular polyfill libraries include core-js, polyfill.io, and Babel with @babel/polyfill. These libraries contain polyfills for many modern JavaScript features, making it easy to add support for older browsers. In short, polyfills bridge the gap between modern JavaScript features and older browsers by adding missing functionality, ensuring your code works everywhere.",
    category: "javascript",
  },
  {
    id: 2009,
    ...
    answer:
      "A generator function is a special type of function that can pause execution and resume later, allowing it to produce a sequence of values over time. Generator functions are defined using the function* syntax with an asterisk, and they use the yield keyword to pause and return values. When you call a generator function, it doesn't execute immediately but instead returns a generator object that you can use to control the execution.\n\nFor example, if you create a numberGenerator function that yields 1, 2, and 3, calling the function gives you a generator object. Each time you call the next() method on this object, the function executes until it reaches a yield statement, returns that value along with a done property indicating whether the generator is finished, and then pauses. The next call to next() resumes execution from where it left off. Key features of generator functions include the ability to pause execution using yield, maintain state between calls, return an iterator object, and resume execution with the next() method.\n\nGenerator functions are useful for lazy evaluation where values are generated on demand rather than all at once, implementing custom iterators, handling infinite sequences without running out of memory, and managing asynchronous operations with async generators. For instance, you can create a generator that produces infinite numbers by using a while true loop with yield, and it won't freeze your program because values are only generated when requested. In short, generator functions can pause and resume execution, making them perfect for handling sequences, lazy evaluation, and complex iteration patterns.",
    category: "javascript",
  },
  {
    id: 2010,
    ...
    answer:
      "IIFE stands for Immediately Invoked Function Expression. It's a function that runs as soon as it is defined. The syntax involves wrapping a function in parentheses and then immediately calling it with another set of parentheses at the end. You can also use arrow functions to create an IIFE. There are several reasons to use IIFE in JavaScript. First, it helps avoid global scope pollution because variables defined inside an IIFE are private and don't leak to the global scope. This creates data encapsulation and privacy, similar to private variables in other languages.\n\nSecond, an IIFE allows you to run code immediately without having to call the function explicitly, which is useful for initialization code. Third, before ES6 modules, developers used IIFE to create isolated modules and prevent naming conflicts. For example, if you declare a variable called secret inside an IIFE and try to access it outside, you'll get an error because the variable only exists within the IIFE's scope. Real-world uses of IIFE include protecting variables from global scope, initializing code that should run once, creating closures, and implementing the old-style module pattern before ES6 modules became standard. You can also pass parameters to an IIFE, making it flexible for various use cases. In short, IIFE is a function that executes immediately and keeps variables private, preventing global scope pollution and providing a clean way to execute initialization code.",
    category: "javascript",
  },
  {
    id: 2011,
    ...
    answer:
      "CORS stands for Cross-Origin Resource Sharing. It's a security feature implemented by browsers to control how web pages from one domain can request resources from another domain. To understand CORS, you first need to know about the Same-Origin Policy, which is a security measure where browsers block requests between different origins for security reasons. An origin is defined as the combination of protocol, domain, and port, such as https://example.com:443.\n\nCORS exists to allow controlled access to resources across different domains while preventing malicious websites from stealing data. It enables legitimate cross-origin requests such as calling APIs from different domains or loading resources from CDNs. CORS works by requiring the server to explicitly allow cross-origin requests by sending specific CORS headers in the response. Common CORS headers include Access-Control-Allow-Origin which specifies which origins can access the resource, Access-Control-Allow-Methods which defines allowed HTTP methods, Access-Control-Allow-Headers which specifies allowed request headers, and Access-Control-Allow-Credentials which indicates whether credentials like cookies can be sent.\n\nWhen a CORS error occurs, you typically see a message stating that access to fetch from one origin has been blocked by CORS policy. Solutions to CORS issues include configuring your server to send proper CORS headers, using a proxy server to relay requests, or enabling CORS in your backend by adding middleware that sets the appropriate headers. For example, in Express, you can add middleware that sets the Access-Control-Allow-Origin header to allow requests from specific or all origins. In short, CORS is a security mechanism that controls cross-origin requests between different domains, balancing security with the need for legitimate cross-domain communication.",
    category: "javascript",
  },
  {
    id: 2012,
    ...
    answer:
      "JavaScript has two main categories of data types: Primitive and Non-Primitive (Reference) types. Primitive data types include seven types. First is Number, which represents both integers and floating-point numbers like 42 or 3.14. Second is String, which represents text data enclosed in quotes, such as 'John' or \"Hello\". Third is Boolean, which has only two values: true or false. Fourth is Undefined, which is automatically assigned to variables that are declared but not given a value. Fifth is Null, which represents the intentional absence of a value and is explicitly set by the programmer. Sixth is Symbol, introduced in ES6, which creates unique and immutable identifiers useful for object properties. Seventh is BigInt, added in ES2020, which allows representation of integers larger than the Number type can handle, denoted with an 'n' at the end.\n\nNon-Primitive or Reference data types include objects, which are collections of key-value pairs; arrays, which are ordered collections of values; functions, which are executable code blocks; Date objects for handling dates and times; and RegExp for working with regular expressions. The key difference between primitive and non-primitive types is that primitives are immutable and stored by value, meaning when you assign a primitive to another variable, a copy is made. Non-primitive types are mutable and stored by reference, meaning variables hold references to the memory location rather than the actual value. In short, JavaScript has 7 primitive types for simple values and reference types like objects, arrays, and functions for complex data structures.",
    category: "javascript",
  },
  {
    id: 2013,
    ...
    answer:
      "TypeScript and JavaScript are closely related, but TypeScript adds extra features on top of JavaScript. JavaScript is dynamically typed, meaning it has no type checking and variables can hold any type of value. It runs directly in browsers without any compilation step, making it flexible but also error-prone since type-related errors only appear at runtime. JavaScript is easier to get started with because you don't need to learn type annotations or deal with compilation. For example, you can assign a number to a variable and later reassign it to a string without any errors, though this can lead to bugs.\n\nTypeScript, on the other hand, is statically typed with type checking built in. It's a superset of JavaScript, meaning all valid JavaScript code is also valid TypeScript. However, TypeScript code must be compiled to JavaScript before it can run in browsers. The major advantage is that TypeScript catches errors at compile time rather than runtime, preventing many common bugs before the code even runs. TypeScript also provides better IDE support with features like autocomplete, refactoring tools, and inline documentation. It supports advanced features like interfaces, enums, generics, and decorators that make it better suited for large-scale applications. For instance, if you declare a variable as a number type in TypeScript and try to assign a string to it, you'll get a compile-time error immediately.\n\nThe key differences between them are: JavaScript uses dynamic typing while TypeScript uses static typing; JavaScript doesn't need compilation while TypeScript compiles to JavaScript; JavaScript detects errors at runtime while TypeScript catches them at compile time; JavaScript has an easier learning curve while TypeScript is steeper; JavaScript files use .js extension while TypeScript uses .ts; JavaScript has no type safety while TypeScript enforces it; and TypeScript generally has better tooling support. Use JavaScript for small projects, quick prototypes, or when learning, and use TypeScript for large applications, team projects, or long-term maintenance. In short, TypeScript equals JavaScript plus static types plus better tooling, helping catch bugs early and improving overall code quality.",
    category: "typescript",
  },
  {
    id: 2014,
    ...
    answer:
      "Authentication and Authorization are two different security concepts often used together but serving distinct purposes. Authentication is the process of verifying the identity of a user, essentially confirming that you are who you say you are. It usually involves logging in with a username and password, and it happens first before any authorization checks. For example, when a user logs in with their email and password, the system verifies these credentials against stored data. If the credentials match, the user is authenticated and typically receives a token such as a JWT or session cookie to maintain their logged-in state. Common authentication methods include password-based authentication, biometric verification like fingerprint or face ID, One-Time Passwords (OTP), OAuth for third-party logins like Google or Facebook, and Multi-Factor Authentication (MFA) which requires multiple forms of verification.\n\nAuthorization, on the other hand, is the process of verifying what a user has access to after they've been authenticated. It determines permissions and access rights, controlling what resources a user can access or what actions they can perform. For instance, an admin might have permission to delete users, a regular user might only be able to view content, and a guest might not have access to premium features at all. Authorization happens after authentication and commonly uses methods like Role-Based Access Control (RBAC) where permissions are assigned based on user roles, Attribute-Based Access Control (ABAC) which uses user attributes to determine access, or general permission-based systems.\n\nA real-world analogy would be airport security: authentication is like showing your ID to prove who you are, while authorization is like your boarding pass showing which specific flight you're allowed to board. In web development, authentication typically involves a login endpoint that verifies credentials and returns a token, while authorization involves middleware that checks if the authenticated user has the required permissions before allowing access to specific resources. In short, authentication verifies identity through login, while authorization verifies permissions through access control.",
    category: "javascript",
  },
  {
    id: 2015,
  ...
  {
    id: 2016,
    question: "Slice vs Splice in javascript?",
    answer:
      "slice() and splice() are both array methods, but they work very differently. The slice method extracts a portion of an array and returns a new array without modifying the original array. It takes two parameters: a start index and an end index, where the end index is not included in the result. For example, if you have an array [1, 2, 3, 4, 5] and call slice(1, 4), it returns a new array [2, 3, 4] while the original array remains unchanged as [1, 2, 3, 4, 5]. This makes slice a safe method for extracting portions of arrays because it doesn't affect the original data.\n\nThe splice method, on the other hand, is used to add, remove, or replace elements in an array and it modifies the original array directly. It returns an array containing the removed elements. The splice method takes three types of parameters: a start index indicating where to begin, a delete count specifying how many elements to remove, and optional items to add at that position. For instance, if you have an array [1, 2, 3, 4, 5] and call splice(1, 2, 'a', 'b'), it removes 2 elements starting at index 1 (which are 2 and 3), adds 'a' and 'b' at that position, returns [2, 3] as the removed elements, and modifies the original array to [1, 'a', 'b', 4, 5].\n\nThe key differences are that slice does not modify the original array while splice does, slice returns a new array subset while splice returns the removed elements, slice is used for extraction while splice is used for adding, removing, or replacing elements, and their parameters differ with slice taking start and end indexes while splice takes start index, delete count, and items to add. Common use cases for slice include copying part of an array or getting a subset without changing the original. Common use cases for splice include removing elements with arr.splice(2, 1), adding elements with arr.splice(2, 0, 'new'), or replacing elements with arr.splice(2, 1, 'new'). In short, slice extracts without modifying the array making it safe, while splice modifies the original array making it destructive.",
    category: "javascript",
  },
  {
    id: 2017,
    ...
    answer:
      "Destructuring is an ES6 feature that allows you to extract values from arrays or properties from objects and assign them to variables in a clean and concise way. Before destructuring, if you wanted to extract values from an array, you had to access each element by its index individually, like const a = arr[0] and const b = arr[1]. With destructuring, you can write const [a, b, c] = [1, 2, 3] to extract all three values in one line. You can also skip elements you don't need by leaving empty spaces in the destructuring pattern, such as const [first, , third] = [1, 2, 3] which gives you 1 and 3 while skipping 2. The rest pattern allows you to collect remaining elements into a new array, like const [first, ...rest] = [1, 2, 3, 4] which gives you first as 1 and rest as [2, 3, 4].\n\nObject destructuring works similarly. The old way of extracting object properties required you to access each property individually like const name = user.name and const age = user.age. With destructuring, you can write const { name, age } = { name: 'John', age: 25 } to extract both properties at once. You can rename variables during destructuring using the syntax const { name: userName, age: userAge } = user, which creates variables with different names than the original properties. You can also provide default values for properties that might not exist, such as const { name, country = 'USA' } = { name: 'John' }, which uses 'USA' as the default value for country. Nested destructuring allows you to extract values from nested objects in one statement, like extracting a city from a nested address object.\n\nDestructuring works great with function parameters, allowing you to extract specific properties from an object argument directly in the parameter list. The benefits of destructuring include cleaner and more readable code, less repetition, easy extraction of nested values, better function parameter handling, and the ability to easily swap variables. For example, you can swap two variables with [a, b] = [b, a] without needing a temporary variable. In short, destructuring is a convenient way to extract values from arrays and objects and assign them to variables in one line, making code more concise and readable.",
    category: "javascript",
  },
  {
    id: 2018,
    question: "What is a callstack in javascript?",
    answer:
      "The call stack is a data structure that JavaScript uses to keep track of function calls in a program. It operates on the Last In, First Out (LIFO) principle, meaning the last function that gets pushed onto the stack is the first one to be executed and removed. When a JavaScript program starts executing, the global execution context is created and placed on the call stack. Whenever a function is called, a new execution context for that function is created and pushed onto the top of the stack. When a function completes its execution and returns, its execution context is popped off the stack, and control returns to the previous context.\n\nFor example, if function A calls function B, and function B calls function C, the call stack would look like this: Global Context at the bottom, then A, then B, then C at the top. When C finishes, it's popped off, then B finishes and is popped off, then A finishes and is popped off, leaving only the global context. JavaScript is single-threaded, which means it has only one call stack and can execute only one function at a time. If a function takes too long to execute or if there's an infinite recursion, the call stack can overflow, resulting in a \"Maximum call stack size exceeded\" error. Understanding the call stack is crucial for debugging and understanding how JavaScript handles function execution, especially when dealing with asynchronous operations where callbacks are placed back onto the call stack by the event loop after async tasks complete.",
    category: "javascript",
  },
  {
    id: 2019,
    ...
    answer:
      "Higher Order Functions are functions that either take one or more functions as arguments, return a function as their result, or both. They are a fundamental concept in functional programming and are heavily used in JavaScript. Higher order functions enable code reusability, abstraction, and the creation of more flexible and maintainable code. Common built-in higher order functions in JavaScript include map, filter, reduce, forEach, sort, and find, all of which accept callback functions as arguments.\n\nFor example, the map function is a higher order function because it takes a function as an argument and applies it to each element in an array. When you write array.map(x => x * 2), the map function takes the arrow function as an argument and uses it to transform each element. You can also create your own higher order functions. For instance, you could create a function called repeat that takes a function and a number, and calls that function the specified number of times. Higher order functions are powerful because they allow you to abstract common patterns, such as iteration, filtering, or transformation, into reusable utilities. They also enable functional programming techniques like function composition, where you combine multiple functions to create more complex operations. In short, higher order functions are functions that work with other functions, either by taking them as parameters or returning them, making your code more modular and expressive.",
    category: "javascript",
  },
  {
    id: 2020,
    question: "What is the difference between Indexeddb and sessionstorage?",
    answer:
      "IndexedDB and sessionStorage are both client-side storage mechanisms, but they differ significantly in capacity, data structure, persistence, and use cases. sessionStorage is a simple key-value storage that stores data as strings and is limited to about 5-10MB depending on the browser. The data stored in sessionStorage persists only for the duration of the page session, meaning it gets cleared when the tab or window is closed. It's synchronous and easy to use with simple methods like setItem and getItem, making it suitable for temporary data like form inputs, UI state, or session-specific preferences.\n\nIndexedDB, on the other hand, is a much more powerful and complex database that can store large amounts of structured data, including objects, files, and blobs, with storage capacity typically ranging from 50MB to hundreds of megabytes or even unlimited in some browsers. Unlike sessionStorage, IndexedDB data persists even after the browser is closed and remains until explicitly deleted by the user or the application. IndexedDB is asynchronous, which means operations don't block the main thread, making it suitable for storing large datasets without affecting performance. It supports indexes, transactions, and queries, functioning more like a real database. IndexedDB is ideal for offline applications, caching large datasets, storing user-generated content, or building progressive web apps that need to work without an internet connection. In short, use sessionStorage for small, temporary data that should last only for the current session, and use IndexedDB for large, persistent, structured data that needs database-like capabilities.",
    category: "javascript",
  },
  {
    id: 2021,
    ...
    answer:
      "Interceptors are functions that allow you to intercept and modify HTTP requests or responses before they are handled by then or catch in your application. They are commonly used in HTTP client libraries like Axios, Angular's HttpClient, and other API communication tools. Interceptors act as middleware that sits between your application code and the actual HTTP request or response, giving you a centralized place to handle common tasks like adding authentication tokens, logging, error handling, transforming data, or showing loading indicators.\n\nThere are two types of interceptors: request interceptors and response interceptors. Request interceptors run before a request is sent to the server, allowing you to modify the request configuration, add headers like authorization tokens, log the request details, or cancel the request based on certain conditions. For example, you might use a request interceptor to automatically attach a JWT token to every API call, so you don't have to manually add it in every request. Response interceptors run after receiving a response from the server but before the data reaches your application code. They allow you to transform response data, handle errors globally, refresh authentication tokens when they expire, or log response details.\n\nInterceptors are powerful because they provide a single place to implement cross-cutting concerns that affect multiple API calls. For instance, instead of writing error handling code in every API call, you can write it once in a response interceptor. Common use cases include adding authentication headers to all requests, transforming date strings into Date objects automatically, handling 401 unauthorized errors globally by redirecting to a login page, showing toast notifications for errors, implementing request/response logging for debugging, and handling token refresh logic when access tokens expire. In short, interceptors are middleware functions that let you globally intercept and modify HTTP requests and responses, making your code cleaner and more maintainable.",
    category: "javascript",
  },
  {
    id: 2022,
    question: "Differences between Promise.all, allSettled, any, race?",
    answer:
      "Promise.all, Promise.allSettled, Promise.any, and Promise.race are all methods for handling multiple promises concurrently, but they behave differently based on how they handle fulfilled and rejected promises. Promise.all takes an array of promises and waits for all of them to resolve. If all promises succeed, it returns an array of all resolved values in the same order. However, if any single promise rejects, Promise.all immediately rejects with that error and ignores the rest. This makes it useful when you need all operations to succeed, such as fetching data from multiple APIs where you need all the data before proceeding. If even one fails, the entire operation is considered failed.\n\nPromise.allSettled also takes an array of promises and waits for all of them to complete, but unlike Promise.all, it doesn't reject if some promises fail. Instead, it waits for all promises to settle, whether they resolve or reject, and returns an array of objects describing the outcome of each promise with a status of either 'fulfilled' or 'rejected' and the corresponding value or reason. This is useful when you want to know the result of all operations regardless of whether they succeeded or failed, such as when sending multiple independent requests and you want to process both successful and failed results.\n\nPromise.race returns as soon as the first promise settles, whether it resolves or rejects. It essentially races all the promises against each other and returns the result of whichever finishes first. This is useful for implementing timeouts, where you race an actual operation against a timeout promise, or for showing the fastest result from multiple redundant sources. Promise.any, introduced in ES2021, waits for the first promise to successfully resolve and returns that value. If all promises reject, it throws an AggregateError containing all the rejection reasons. Unlike Promise.race which returns the first settled promise regardless of success or failure, Promise.any specifically waits for the first successful resolution, making it useful when you have multiple fallback options and you want to use whichever succeeds first. In summary, use Promise.all when you need all promises to succeed, Promise.allSettled when you need results from all promises regardless of success or failure, Promise.race when you need the first settled promise, and Promise.any when you need the first successful promise.",
    category: "javascript",
  },
  {
    id: 2023,
    ...
    answer:
      "Arrow functions have several limitations compared to regular functions in JavaScript. First and most importantly, arrow functions do not have their own 'this' binding. They inherit 'this' from the surrounding lexical scope at the time they are defined, which means you cannot use them as methods in objects when you need to access the object's properties using 'this'. If you define an object method as an arrow function and try to access 'this', it won't refer to the object but rather to the outer scope. This also means arrow functions cannot be used as constructors, so you cannot use the 'new' keyword with them. If you try to instantiate an arrow function with 'new', JavaScript will throw an error.\n\nSecond, arrow functions do not have their own 'arguments' object. The 'arguments' object is an array-like object that contains all the arguments passed to a function, and it's available in regular functions but not in arrow functions. If you try to access 'arguments' inside an arrow function, it will refer to the 'arguments' object of the enclosing function if there is one, or it will be undefined. If you need to access all arguments in an arrow function, you should use the rest parameter syntax instead. Third, arrow functions cannot be used as generator functions. You cannot create generator functions using arrow syntax because there is no way to use the function* syntax with arrows.\n\nFourth, arrow functions do not have a prototype property. Regular functions have a prototype property that is used when the function is used as a constructor, but arrow functions don't have this property since they cannot be used as constructors. Fifth, arrow functions cannot be used for methods that need to be dynamically bound. For example, event handlers in classes sometimes need to be bound, and while you can use arrow functions as class properties for automatic binding, you lose some flexibility. These limitations make arrow functions unsuitable for certain scenarios, and you should use regular functions when you need 'this' binding, 'arguments' object, constructor functionality, or generator capabilities. However, arrow functions are perfect for callbacks, array methods, and functional programming patterns where lexical 'this' binding is desired.",
    category: "javascript",
  },
  {
    id: 2024,
    ...
    answer:
      "eval() is a built-in JavaScript function that executes a string of JavaScript code as if it were actual code. It takes a string argument containing JavaScript expressions or statements and evaluates it in the current execution context. For example, if you write eval('2 + 2'), it will return 4, and if you write eval('var x = 10'), it will actually create a variable x with the value 10 in the current scope. While this might seem powerful, eval() is considered dangerous and is strongly discouraged in modern JavaScript development for several important reasons.\n\nFirst, eval() poses significant security risks. If you use eval() with user input or data from untrusted sources, it opens your application to code injection attacks where malicious code can be executed. An attacker could inject harmful JavaScript that steals data, manipulates the DOM, or performs unauthorized actions. Second, eval() is slow because the JavaScript engine cannot optimize code that uses eval(). The engine has to invoke the full JavaScript interpreter every time eval() is called, which is much slower than normal code execution. Third, eval() makes code harder to debug and maintain because it's difficult to track where variables are defined and what code is actually running when strings are being evaluated dynamically.\n\nFourth, eval() can access and modify variables in the local scope, which can lead to unexpected behavior and bugs. If you use eval() in a function, it can create or modify local variables in ways that are hard to predict and track. Modern alternatives to eval() include JSON.parse() for parsing JSON strings safely, Function constructor for creating functions from strings in a safer way with limited scope access, template literals for string interpolation, and proper code architecture that avoids the need for dynamic code evaluation. In short, eval() executes string code dynamically, but it's dangerous, slow, and hard to debug, so it should be avoided in favor of safer alternatives in almost all cases.",
    category: "javascript",
  },
  {
    id: 2025,
  ...
  },
  {
    ...
    answer:
      "Cookies are small pieces of data that are stored in the user's browser and sent to the server with every HTTP request to the same domain. They are primarily used for session management, personalization, and tracking user behavior across websites. When a server sends a response to a browser, it can include a Set-Cookie header that tells the browser to store a cookie. From that point on, the browser automatically includes that cookie in the Cookie header of every subsequent request to the same server, allowing the server to identify the user and maintain state across multiple requests.\n\nCookies have several important attributes that control their behavior. The expires or max-age attribute determines when the cookie will be deleted. If not set, the cookie is considered a session cookie and is deleted when the browser closes. The domain attribute specifies which domain can access the cookie, and the path attribute specifies which URL path the cookie applies to. The secure attribute ensures the cookie is only sent over HTTPS connections, protecting it from being intercepted. The httpOnly attribute prevents JavaScript from accessing the cookie using document.cookie, which helps protect against cross-site scripting (XSS) attacks. The SameSite attribute controls whether the cookie is sent with cross-site requests, helping prevent cross-site request forgery (CSRF) attacks.\n\nCookies are limited to about 4KB of data per cookie, and browsers typically allow around 20-50 cookies per domain. Common use cases for cookies include storing authentication tokens or session IDs to keep users logged in, remembering user preferences like language or theme settings, tracking user behavior for analytics, storing shopping cart items in e-commerce sites, and implementing remember me functionality. However, cookies have some drawbacks: they are sent with every request which can impact performance, they have limited storage capacity, they raise privacy concerns and are subject to regulations like GDPR, and they can be vulnerable to security issues if not properly secured. Modern alternatives for client-side storage include localStorage and sessionStorage for data that doesn't need to be sent to the server, and IndexedDB for larger structured data. In short, cookies are small data pieces stored in the browser and sent with every request, mainly used for authentication, tracking, and maintaining user state across sessions.",
    category: "javascript",
  },
  {
    id: 2027,
    ...
    answer:
      "The typeof operator is a built-in JavaScript operator that returns a string indicating the type of a variable or expression. It's used to check the data type of a value at runtime, which is particularly useful in JavaScript's dynamic type system. The syntax is simple: you write typeof followed by the variable or value you want to check, like typeof x or typeof(x), both of which work the same way. The typeof operator returns one of several possible string values: 'undefined' for undefined values, 'boolean' for true or false, 'number' for all numeric values including integers and floats, 'bigint' for BigInt values, 'string' for text values, 'symbol' for Symbol values, 'function' for functions, and 'object' for objects, arrays, and null.\n\nIt's important to note some quirks of the typeof operator. First, typeof null returns 'object', which is actually a historical bug in JavaScript that has been kept for backwards compatibility. This means you cannot reliably use typeof to check for null. Second, typeof returns 'object' for arrays, which means you cannot distinguish between plain objects and arrays using typeof alone. To check if something is an array, you should use Array.isArray() instead. Third, typeof can safely check undeclared variables without throwing an error. If you use typeof on a variable that was never declared, it returns 'undefined' rather than throwing a ReferenceError, which is unique behavior compared to accessing undeclared variables directly.\n\nCommon use cases for typeof include checking if a variable is undefined before using it, determining if a value is a function before calling it, validating function parameters to ensure they are the expected type, and implementing type guards in defensive programming. However, for more reliable type checking, especially for objects and arrays, it's better to use other methods like Array.isArray(), instanceof, or Object.prototype.toString.call(). In short, typeof is an operator that returns a string representing the type of a value, useful for basic type checking but with some quirks like returning 'object' for null and arrays.",
    category: "javascript",
  },
  {
    id: 2028,
